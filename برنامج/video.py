        ...
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                ...
                if pq == "720" or "480" or "360":
                    ...
                    Q = 720
                    await loser.edit(
                        "» __only 720, 480, 360 allowed__ \n💡 **الآن يتدفقون الفيديو في in 720p**"
                    )
            ...
            if len(m.command) < 2:
                ...
                if search == 0:
                    ...
                    if veez == 0:
                        await loser.edit(f"❌ yt-dl تم اكتشاف مشكلات\n\n» `{ytlink}`")
                    else:
                        if chat_id in QUEUE:
                            pos = add_to_queue(
...
async def vstream(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if len(m.command) == 2:
            ...
            Q = 720
            loser = await m.reply("🔄 **جاري المعالجه...**")
        ...
        if veez == 0:
            await loser.edit(f"❌ yt-dl تم الكشف عن المشكلات\n\n» `{ytlink}`")
        else:
            if chat_id in QUEUE:
